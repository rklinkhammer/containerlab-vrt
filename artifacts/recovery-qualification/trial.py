...
OUT=ROOT/'artifacts/recovery-qualification';OUT.mkdir(parents=True,exist_ok=True)
...
try:
 ...
 # Reproduce the previous negative control and inspect the actual namespace.
 started=now();run(['sudo','docker','stop','--time','5',PREFIX+'detector']);run(['sudo','docker','start',PREFIX+'detector']);time.sleep(12)
 broken=snapshot('docker-restart',started)
 RESULT['negative_control']={'interface_present':broken['detector_interface']['exit']==0,'traffic_ready':ready(broken),'new_boot':broken['boots']!=baseline['boots']}
 # Use the full native topology; never recreate links with ip/netlink commands.
 help_text=(OUT/'deploy-help.txt').read_text()
 if '--dry-run' in help_text:
  RESULT['dry_run_exit']=lifecycle('reconcile-dry-run',['deploy','--topo',TOPO,'--dry-run','--format','json'])
 phase=now();code=lifecycle('reconcile',['deploy','--topo',TOPO,'--format','json'])
 case={'case':'native_reconcile_after_docker_restart','exit':code}
 try:
  assert code==0
  recovered=verify_traffic('reconciled',phase)
  case.update(outcome='PASS',non_target_changes=unchanged(baseline,recovered))
 except Exception as e:case.update(outcome='FAIL',reason=str(e));recovered=None
 ...
 if recovered is not None:
  ...
  try:
   ...
   assert replaced['nodes']['detector']['id']!=recovered['nodes']['detector']['id']
   assert replaced['heartbeat']['boot_id']!=recovered['heartbeat']['boot_id']
   case.update(outcome='PASS',non_target_changes=unchanged(recovered,replaced))
  except Exception as e:case.update(outcome='FAIL',reason=str(e))
  RESULT['cases'].append(case)
 if any(c['outcome']=='FAIL' or c.get('non_target_changes') for c in RESULT['cases']):
  phase=now();code=lifecycle('full-redeploy',['redeploy','--topo',TOPO,'--cleanup','--graceful','--format','json'])
  case={'case':'disruptive_full_redeploy','exit':code}
  try:
   assert code==0
   full=verify_traffic('full-redeploy',phase)
   case.update(outcome='PASS',changed_nodes=[n for n in NODES if baseline['nodes'][n].get('id')!=full['nodes'][n].get('id')])
  except Exception as e:case.update(outcome='FAIL',reason=str(e))
  RESULT['cases'].append(case)
