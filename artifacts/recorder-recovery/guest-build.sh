...
mkdir -p artifacts/recorder-recovery
...
sudo docker image inspect containerlab-vrt-app:local > artifacts/recorder-recovery/image.json
...
sudo docker run --rm --network none -v "$PWD/generated:/generated:ro" containerlab-vrt-tests:qualification \
 ctest --test-dir /tmp/build --output-on-failure | tee artifacts/recorder-recovery/ctest.log
