...
mkdir -p artifacts/processor-recovery-fix
...
sudo docker image inspect containerlab-vrt-app:local > artifacts/processor-recovery-fix/image.json
...
sudo docker run --rm --network none -v "$PWD/generated:/generated:ro" containerlab-vrt-tests:qualification \
 ctest --test-dir /tmp/build --output-on-failure | tee artifacts/processor-recovery-fix/ctest.log
