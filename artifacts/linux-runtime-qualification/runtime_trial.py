...
parser.add_argument("--fault-only", action="store_true", help="Separate fault trial; establishes no sustained-duration result")
args=parser.parse_args()
TARGET_SECONDS=0 if args.fault_only else 340
ROOT=pathlib.Path.home()/'containerlab-vrt'
OUT=ROOT/'artifacts/linux-runtime-qualification'
if args.fault_only: OUT=OUT/'faults'
...
def snapshot(label):
 ...
 for node in NODES:
  ...
  if node!='switch1':
   ...
   heartbeats=[x for x in telemetry if x['event'] in ('heartbeat','shutdown')]
   result[node]['last_telemetry']=heartbeats[-1] if heartbeats else None
...
try:
 ...
 capture=None if args.fault_only else subprocess.Popen(['sudo','bash','scripts/capture.sh'],cwd=ROOT,stdout=(OUT/'capture.log').open('w'),stderr=subprocess.STDOUT)
 while time.monotonic()-start<TARGET_SECONDS:
  time.sleep(max(0,min(30,TARGET_SECONDS-(time.monotonic()-start))))
  print('sustained elapsed',round(time.monotonic()-start),flush=True)
  command(['sudo','docker','ps','--format','{{.Names}} {{.Status}}'])
 if capture is not None: assert capture.wait(timeout=5)==0,'bounded recorder capture failed'
 ...
 # Pause consumes no downstream data but preserves Containerlab-owned links.
 # Process identity reset is tested separately by test_telemetry_process.py.
 command(['sudo','docker','pause',PREFIX+'detector'])
 time.sleep(10)
 snapshot('consumer-paused')
 command(['sudo','docker','unpause',PREFIX+'detector'])
 time.sleep(15)
 restarted=snapshot('consumer-resumed')
 assert len(restarted['detector']['boot_ids'])==1,'pause must not restart the process'
 ...
 snapshot('shutdown')
 (OUT/'trial-status.json').write_text(json.dumps({'completed':True,'sustained_seconds':TARGET_SECONDS,'fault_only':args.fault_only},indent=2)+'\n')
