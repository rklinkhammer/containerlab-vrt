...
mkdir -p artifacts/linux-runtime-qualification
...
sudo docker image inspect containerlab-vrt-app:local > artifacts/linux-runtime-qualification/image.json
...
sudo docker run --rm --network none -v "$PWD/generated:/generated:ro" containerlab-vrt-tests:qualification \
 ctest --test-dir /tmp/build --output-on-failure | tee artifacts/linux-runtime-qualification/ctest.log
python3 scripts/qualify_telemetry_linux.py > artifacts/linux-runtime-qualification/isolated-telemetry.log
